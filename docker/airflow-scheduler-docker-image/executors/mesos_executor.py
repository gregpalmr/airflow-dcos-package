...
class AirflowMesosScheduler(mesos.interface.Scheduler):
    ...
    def __init__(self,
        ...
        if (not configuration.get('mesos', 'USE_DOCKER_CONTAINER')) or (not configuration.get('mesos', 'DEFAULT_DOCKER_CONTAINER_IMAGE')):
            self.use_docker_container = False
            self.default_docker_container_image = 'None'
        else:
            self.use_docker_container = True
            self.default_docker_container_image = configuration.get('mesos', 'DEFAULT_DOCKER_CONTAINER_IMAGE')

        if not configuration.get('core', 'AIRFLOW_HOME'):
            self.airflow_home = '/usr/local/airflow'
        else:
            self.airflow_home = configuration.get('core', 'AIRFLOW_HOME')
    ...
    def resourceOffers(self, driver, offers):
        for offer in offers:
            ...
            while (not self.task_queue.empty()) and \
                ...
                logging.info("Launching task with command: [%s]", cmd)
                ...
                if self.use_docker_container:
                    docker_cmd = "docker run " + self.default_docker_container_image + " /bin/bash -c \"export AIRFLOW_HOME=" + self.airflow_home + " && " + cmd + " \""
                    logging.info('Using Docker Container with command: [%s]', docker_cmd)
                    cmd = docker_cmd
